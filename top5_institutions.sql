...
CA_selection as (select * from (
select distinct institution_name from (
select institution_name, STABBR,foreign_total,sum_total,foreign_perc,year,ifnull(COSTT4_A,0) COSTT4_A,
ifnull(COSTT4_P,0) COSTT4_P,lag,median, rank() over (partition by stabbr order by foreign_total desc) t
from final
where institution_name in (
    ...
    from (
        ...
        from (
            ...
            where institution_name in (
                ...
                from (
                    ...
                    AND (COSTT4_A < median and COSTT4_P < median)
                    )
                )
            )
        ...
        and stabbr = 'CA'
        order by t limit 50)) limit 3),
        
TX_selection as (select * from (
select distinct institution_name from (
select institution_name, STABBR,foreign_total,sum_total,foreign_perc,year,ifnull(COSTT4_A,0) COSTT4_A,
ifnull(COSTT4_P,0) COSTT4_P,lag,median, rank() over (partition by stabbr order by foreign_total desc) t
from final
        ...
        from (
            ...
            where institution_name in (
                ...
                from (
                    ...
                    from final 
                    where ((stabbr = 'TX' and foreign_perc > 0.04) OR (stabbr = 'CA' and foreign_perc > 0.16))
                    -- OR (stabbr = 'CA' and foreign_perc > 0.16)
                    AND (COSTT4_A < median and COSTT4_P < median)
                    )
                )
            )
...
select * from final where institution_name in (select institution_name from (select * from tx_selection))
union all
select * from final where institution_name in (select institution_name from (select * from ca_selection))
